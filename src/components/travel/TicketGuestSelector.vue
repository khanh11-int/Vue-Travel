<template>
  <div ref="rootRef" class="selector">
    ...
    <div v-if="isOpen" class="selector__popup" role="dialog" aria-label="Chọn vé và trẻ em">
      <div class="selector__row">
        <div>
          <p class="selector__label">Số vé</p>
          <p class="selector__hint">Vé người lớn</p>
        </div>
        <div class="selector__counter">
          <button type="button" :disabled="draft.tickets <= 1" @click="draft.tickets = Math.max(1, draft.tickets - 1)">-</button>
          <span>{{ draft.tickets }}</span>
          <button type="button" @click="draft.tickets += 1">+</button>
        </div>
      </div>
      ...
      <div v-if="draft.children > 0" class="selector__ages">
        ...
        <div v-for="(age, index) in draft.childrenAges" :key="`ticket-child-${index}`" class="selector__age-field">
          <label :for="`ticket-child-age-${index}`">Trẻ em {{ index + 1 }}</label>
          <select :id="`ticket-child-age-${index}`" :value="age" @change="updateChildAge(index, $event)">
...
const normalize = (payload = {}) => {
  const tickets = Math.max(1, Math.floor(Number(payload.tickets) || 1))
...
const summary = computed(() => `${draft.tickets} vé, ${draft.children} trẻ em`)
